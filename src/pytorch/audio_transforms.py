...
class LoadAudio(object):
    ...
    def __call__(self, data):
        ...
        data['samples'] = samples.astype(np.float32)
        data['sample_rate'] = sample_rate
        return data


class ToWavFile(object):
    """Writes a numpy array into a WAV file."""
    ...
    def __call__(self, data):
        ...
        if filename:
            file = "{}_{}".format(data['target'], filename)
            path = os.path.join(self.path, file)
            # Must convert from float64 to float32 or else it ain't a real WAV.
            librosa.output.write_wav(
                path, data['samples'].astype(np.float32), self.sample_rate
            )
        return data
